...
function playerLogin.onLogin(player)
	...
	if player:getLastLoginSaved() == 0 then
		...
		if backpack then
			for i = 1, #items do
				backpack:addItem(items[i][1], items[i][2], items[i][3], items[i][4], items[i][5], items[i][6], items[i][7], items[i][8], items[i][9], items[i][10], items[i][11], items[i][12])
			end
		end
		...
		player:addItem(2467, 1, true, 1, CONST_SLOT_ARMOR)
